...
fn seed_accounts(node_url: String, alice: &SrPair, sender_accounts: &[SrPair]) {
...
async fn start_sending(
	...
	loop {
		...
		if process_block_metrics_and_log(
			best_block_r,
			&mut timestamp,
			&mut block_time,
			&mut tps_window,
...
async fn process_block_metrics_and_log(
	...
	let btime_ms = if block_time.is_zero() { 6000 } else { block_time.as_millis() } as u64;
...
fn main() -> Result<(), Box<dyn Error>> {
	...
	if args.seed {
		seed_accounts(args.node_url.clone(), &alice, &sender_accounts);
	}
