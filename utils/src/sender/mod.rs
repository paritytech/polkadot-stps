use subxt::{ClientBuilder, PolkadotExtrinsicParams, PolkadotExtrinsicParamsBuilder as Params, PairSigner, DefaultConfig, sp_core::{Pair, sr25519::Pair as SrPair}, extrinsic::Era};
use log::*;
...
pub async fn send_funds(node: &String, derivation: &str, chunk_size: usize, n: usize) -> Result<(), Box<dyn std::error::Error>> {
    // todo: spawn thread
...
pub fn generate_signer(derivation_blueprint: &str, i: usize) -> PairSigner<DefaultConfig, SrPair> {
    let pair: SrPair = Pair::from_string(format!("{}{}", derivation_blueprint, i).as_str(), None).unwrap();
